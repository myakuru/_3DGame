...
void Scoring::Update()
{
	...
	float delataTime = Application::Instance().GetUnscaledDeltaTime();
	...
	{
		if (m_position.y > m_dawnPos.y) m_position.y -= m_dawnTimer * delataTime;
	}
...
void Scoring::JsonInput(const nlohmann::json& _json)
{
